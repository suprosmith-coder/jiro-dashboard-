...
class Moderation(commands.Cog):
    ...
    async def purge_prefix(self, ctx, target=None, amount: int = 100):
    ...
    async def purge_error(self, ctx, error):
        # If "banned" was passed, discord.py raises MemberNotFound — re-dispatch correctly
        if isinstance(error, commands.MemberNotFound):
            raw = ctx.message.content.split()
            if len(raw) >= 2 and raw[1].lower() == "banned":
                return await ctx.invoke(self.purge_prefix, target="banned")
            # Could also be a plain number e.g. ?purge 50
            if len(raw) >= 2 and raw[1].isdigit():
                return await ctx.invoke(self.purge_prefix, target=None, amount=int(raw[1]))
        # All other errors go to the global handler
        raise error
